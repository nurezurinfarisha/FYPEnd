document.addEventListener("DOMContentLoaded", () => {
    const songListContainer = document.getElementById("scrollable-song-list");
    const audioPlayer = document.getElementById("audioPlayer");
    const recommendationsContainer = document.getElementById("recommendations-container");
    const nowPlayingContainer = document.getElementById("now-playing-container");
    const nowPlayingTitle = document.getElementById("now-playing-title");
    const nowPlayingImg = document.getElementById("now-playing-img");
    const lyricsContent = document.getElementById("lyrics-content2");
const songs = [
    {
        "id": 1,
        "title": "The Lazy Song",
    ...
    document.getElementById("shuffle-button").addEventListener("click", () => {
        ...
        loadSongList(); // Reload the shuffled song list
    });
        // Function to fetch and display lyrics
        async function fetchAndDisplayLyrics(songTitle) {
            try {
                const response = await fetch(`/get-lyrics?song=${encodeURIComponent(songTitle)}`);
                const data = await response.json();
        
                console.log("Fetched Lyrics Data:", data); // Debug log
        
                if (data.lyrics) {
                    lyricsContent.innerHTML = data.lyrics.replace(/\n/g, "<br>");
                    console.log("Lyrics HTML:", lyricsContent.innerHTML); // Debug log
                    enableScrollingAnimation();
                } else {
                    lyricsContent.innerHTML = "<p>No lyrics available for this song.</p>";
                }
            } catch (error) {
                console.error("Error fetching lyrics:", error);
                lyricsContent.innerHTML = "<p>Error loading lyrics. Please try again later.</p>";
            }
        }
        ...
        function enableScrollingAnimation() {
            lyricsContent.style.animation = "scrollLyrics2 120s linear infinite"; // Adjust duration as needed
        }

    function displayRecommendations(currentSongId) {
        recommendationsContainer.innerHTML = ""; // Clear previous recommendations
    
        const currentIndex = songs.findIndex((song) => song.id === currentSongId);
    
        if (currentIndex >= 0) {
            // Get all songs after the current one
            const nextSongs = songs.slice(currentIndex + 1);
    
            if (nextSongs.length > 0) {
                nextSongs.forEach((song) => {
                    const recommendationCard = document.createElement("div");
                    recommendationCard.className = "recommendation-item";
                    recommendationCard.innerHTML = `
                        <div class="recommendation-info" onclick="playSong(${song.id})" style="cursor: pointer;">
                ...
                recommendationsContainer.innerHTML = "<p>No more songs available.</p>";
            }
        } else {
            recommendationsContainer.innerHTML = "<p>Unable to find current song.</p>";
        }
    }
    
    

    // Play the selected song and update recommendations
    window.playSong = function (songId) {
        const selectedSong = songs.find((song) => song.id === songId);
        if (selectedSong) {
            // Update the "Now Playing" section
            nowPlayingTitle.textContent = selectedSong.title;
            nowPlayingImg.src = selectedSong.image;
            fetchAndDisplayLyrics(selectedSong.title);

            // Set the audio source and play the song
            const audioSource = document.getElementById("audio-source");
            audioSource.src = selectedSong.file_path;
            const audioPlayer = document.getElementById("audio-player");
            audioPlayer.load();
            audioPlayer.play();

            // Display the next recommendations
            displayRecommendations(songId);
        ...
        songs.forEach((song) => {
            ...
            songCard.innerHTML = `
                <div class="song-album-container">
                    <img src="${song.image}" alt="${song.title}" class="song-album-image" />
                    <div class="song-hover-overlay">
