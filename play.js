document.addEventListener('DOMContentLoaded', function () {
    ...
    const audioPlayer = document.getElementById('audioPlayer'); // Ensure this ID matches your audio player element
    const analyzingPopup = document.getElementById('analyzingPopup'); // ID for your pop-up element
    const songs = [
        {
            "id": 1,
            "title": "The Lazy Song",
        ...
        },
        {
            ...
            "image": "/static/images/cover/83.jpeg",
        ...
        },
        {
            "id": 115,
            "title": "Say Something",
            "formatted_title": "Say Something Lyrics",
        ...
        },
        {
            ...
            "formatted_title": "Its Always Been You Official Lyric Video",
        ...
        },
        {
            ...
            "formatted_title": " Rolling In The Deep Official Music Video",
            "image": "/static/images/cover/173.jpeg",
            "file_path": "/uploads/A51_Rolling_in_the_Deep_Official_Music_Video.mp3"
        }
    ];

    // Shuffle function to randomize the order of songs
    ...
    // Function to render songs on the page
...
function renderSongs() {
    const songList = document.querySelector('.song-list'); // Ensure songList is defined
    songList.innerHTML = ''; // Clear existing songs
    shuffleSongs(songs); // Shuffle the songs array
    ...
    songs.forEach(song => {
        ...
        songItem.innerHTML = `
            <div class="song-info">
                <img src="${song.image}" alt="Album Art" class="album-art">
                <div class="song-details">
                    <p>${song.formatted_title}</p>
                    <button class="play-btn" data-song-id="${song.id}" data-file-path="${song.file_path}">
    ...
    addPlayButtonEventListeners(); // Add event listeners for the new play buttons
}

// Shuffle songs and re-render on button click
document.getElementById('shuffle-button').addEventListener('click', function () {
    renderSongs(); // Call renderSongs to re-display shuffled songs
});

// Initial load when the page is first loaded
document.addEventListener("DOMContentLoaded", function () {
    renderSongs(); // Render songs initially on page load
});
    ...
    function hideAnalyzingPopup() {
        if (analyzingPopup) {
            analyzingPopup.style.display = 'none';
        }
    }

    // Function to add play button event listeners
    function addPlayButtonEventListeners() {
        document.querySelectorAll('.play-btn').forEach(button => {
            button.addEventListener('click', async function () {
                ...
                try {
                    const response = await fetch(`/analyze/rhythm/${songId}`);
                    if (response.ok) {
                        // Optionally redirect after analysis completes
                        window.location.href = `/analyze/rhythm/${songId}`;
                        hideAnalyzingPopup();
                    } else {
